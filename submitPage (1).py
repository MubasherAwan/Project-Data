# -*- coding: utf-8 -*-
"""
Created on Wed Jun  1 23:07:00 2022
...
prophetNamesReplacerList = ["رسول الله صلى الله عليه وسلم", "النبي صلى الله عليه وسلم", "يا رسول الله","محمد صلی اللہ علیہ وآلہ وسلم"]
...
for i in range(len(narratorsNameList)):
    ...
    myDb = mysql.connector.connect(host="localhost", user="root", passwd="Alibaqar_98680", database="scholar_database")
...
for j in range(len(finalInfoList_Narrators)):
    ...
    if (len(finalInfoList_Narrators[j]) == 7):
...
for i in range(len(finalInfoList_Narrators)):
    if (len(finalInfoList_Narrators[i]) == 7):
...
params = {'hadithText': withoutAerabText, 'list': showNarratorsList,'typeAccordingToNisbat':typeAccordingToNisbat,'reasonAccordingToNisbat':reasonAccordingToNisbat}
